...
event NewProposal(address sender, uint amount);
...
function getAllProposals()public view returns(Proposal [] memory){
    Proposal[] memory tempProposal = new Proposal[](proposalsCount);
    for(uint256 index = 0; index < proposalsCount; index++){
        tempProposal[index] = proposals[index];
...
function getVotes()public view onlyStakeholder("Only Stakeholder can call this function.") returns(uint256[] memory){
return votes[msg.sender];

}

function getStakeholderBal()public view 
...
function getMemberBal()public view onlyMember("Only Members can call this function.") returns(uint256){
    return members[msg.sender];
